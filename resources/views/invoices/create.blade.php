    ...
    <div class="max-w-6xl mx-auto space-y-6" x-data="invoiceBuilder()">
        ...
        <form action="{{ route('invoices.store') }}" method="POST" @submit="validateForm($event)" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                    <div>
                        ...
                        <select name="sale_type" x-model="saleType" class="w-full px-3.5 py-2 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-brand-500 focus:outline-none">
                            <option value="LOCAL">Intra-State: Bihar (CGST + SGST)</option>
            ...
            <div class="p-6 rounded-2xl bg-white border border-slate-200/80 shadow-sm space-y-4">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <template x-for="(item, index) in items" :key="index">
                                <tr class="hover:bg-slate-50/50">
                                    ...
                                    <td class="py-2 px-3">
                                        <input type="number" step="0.01" min="0" :name="`items[${index}][rate]`" x-model.number="item.rate" required placeholder="0.00"
                                               @keydown.enter.prevent="addItem()"
                ...
                <!-- Summary & Total Calculation Bar -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-100">
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Customer Notes / Payment Terms</label>
                        <textarea name="notes" rows="3" placeholder="Thank you for your business! Bank transfer or UPI accepted."
                                  class="w-full px-3.5 py-2 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-brand-500 focus:outline-none"></textarea>
                    </div>
                    ...
                    <div class="p-4 rounded-xl bg-slate-50 border border-slate-200 font-mono text-sm space-y-2">
                        ...
                        <!-- Dynamic Display depending on simple vs detailed tax mode -->
            ...
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('invoices.index') }}" class="px-5 py-2.5 rounded-xl border border-slate-300 text-slate-700 text-sm font-semibold hover:bg-slate-50">
                    Cancel
                </a>
